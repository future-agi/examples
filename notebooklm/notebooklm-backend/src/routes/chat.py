...
def list_conversations(notebook_id):
    ...
    with tracer.start_as_current_span("list_conversations") as span:
...
def get_conversation(notebook_id, conversation_id):
    ...
    with tracer.start_as_current_span("get_conversation") as span:
        span.set_attribute(SpanAttributes.FI_SPAN_KIND, FiSpanKindValues.CHAIN.value)
        span.set_attribute("input.value", json.dumps({"notebook_id": notebook_id, "conversation_id": conversation_id}))
...
def search_notebook(notebook_id):
    ...
    with tracer.start_as_current_span("search_notebook") as span:
        span.set_attribute(SpanAttributes.FI_SPAN_KIND, FiSpanKindValues.CHAIN.value)
        span.set_attribute("input.value", json.dumps({"notebook_id": notebook_id}))
        ...
        try:
            ...
            span.set_attribute("output.value", json.dumps({
                ...
                "error": {
                    "code": "INTERNAL_ERROR",
                    "message": "Failed to search notebook",
                    "details": str(e)
                }
            }))
            return jsonify({
