...
class DocumentProcessor:
    ...
    def process_document(self, file_path: str = None, url: str = None, 
        ...
        with tracer.start_as_current_span("process_document") as span:
            ...
            span.set_attribute("input.value", json.dumps({"file_path": file_path, "url": url, "content": content, "doc_type": doc_type}))
            """
    ...
    def _process_pdf(self, file_path: str, url: str = None) -> Dict:
        with tracer.start_as_current_span("process_pdf") as span:
            span.set_attribute(SpanAttributes.FI_SPAN_KIND, FiSpanKindValues.TOOL.value)
            span.set_attribute("input.value", json.dumps({"file_path": file_path, "url": url}))
            """Process PDF files"""
            try:
    ...
    def _process_docx(self, file_path: str, url: str = None) -> Dict:
        with tracer.start_as_current_span("process_docx") as span:
            span.set_attribute(SpanAttributes.FI_SPAN_KIND, FiSpanKindValues.TOOL.value)
            span.set_attribute("input.value", json.dumps({"file_path": file_path, "url": url}))
            """Process DOCX files"""
            try:
    ...
    def _process_txt(self, file_path: str, url: str = None) -> Dict:
        with tracer.start_as_current_span("process_txt") as span:
            span.set_attribute(SpanAttributes.FI_SPAN_KIND, FiSpanKindValues.TOOL.value)
            span.set_attribute("input.value", json.dumps({"file_path": file_path, "url": url}))
            ...
            try:
                ...
                span.set_attribute("output.value", json.dumps(self._process_text_content(content)))
    ...
    def _process_markdown(self, file_path: str, url: str = None) -> Dict:
        with tracer.start_as_current_span("process_markdown") as span:
            span.set_attribute(SpanAttributes.FI_SPAN_KIND, FiSpanKindValues.TOOL.value)
            span.set_attribute("input.value", json.dumps({"file_path": file_path, "url": url}))
    ...
    def _process_youtube(self, file_path: str = None, url: str = None) -> Dict:
        with tracer.start_as_current_span("process_youtube") as span:
            span.set_attribute(SpanAttributes.FI_SPAN_KIND, FiSpanKindValues.TOOL.value)
            span.set_attribute("input.value", json.dumps({"file_path": file_path, "url": url}))
    ...
    def _process_text_content(self, content: str) -> Dict:
        with tracer.start_as_current_span("process_text_content") as span:
            span.set_attribute(SpanAttributes.FI_SPAN_KIND, FiSpanKindValues.TOOL.value)
            span.set_attribute("input.value", json.dumps({"content": content}))
    ...
    def _chunk_text(self, text: str, chunk_size: int = 1000, overlap: int = 200) -> List[Dict]:
        with tracer.start_as_current_span("chunk_text") as span:
            span.set_attribute(SpanAttributes.FI_SPAN_KIND, FiSpanKindValues.TOOL.value)
            span.set_attribute("input.value", json.dumps({"text": text, "chunk_size": chunk_size, "overlap": overlap}))
            """
    ...
    def _extract_youtube_id(self, url: str) -> Optional[str]:
        with tracer.start_as_current_span("extract_youtube_id") as span:
            span.set_attribute(SpanAttributes.FI_SPAN_KIND, FiSpanKindValues.TOOL.value)
            ...
            for pattern in patterns:
                ...
                if match:
                    span.set_attribute("output.value", json.dumps(match.group(1)))
                    return match.group(1)
            span.set_attribute("output.value", json.dumps(None))
            return None
