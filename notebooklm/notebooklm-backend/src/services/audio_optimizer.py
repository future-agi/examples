...
class AudioOptimizer:
    ...
    def process_whisper_transcript(result: Dict) -> Tuple[str, List[Dict]]:
        with tracer.start_as_current_span("process_whisper_transcript") as span:
            span.set_attribute(SpanAttributes.FI_SPAN_KIND, FiSpanKindValues.TOOL.value)
            span.set_attribute("input.value", json.dumps({"result": result}))
            """
            ...
            span.set_attribute("output.value", json.dumps({"clean_text": clean_text, "segments_with_timestamps": segments_with_timestamps}))
    ...
    def create_smart_chunks(text: str, segments: List[Dict], 
        ...
        with tracer.start_as_current_span("create_smart_chunks") as span:
            span.set_attribute(SpanAttributes.FI_SPAN_KIND, FiSpanKindValues.TOOL.value)
            span.set_attribute("input.value", json.dumps({"text": text, "segments": segments, "chunk_size": chunk_size, "overlap": overlap}))
            """
            ...
            span.set_attribute("output.value", json.dumps(chunks))
            return chunks
    
    @staticmethod
    def _split_into_sentences(text: str) -> List[str]:
        with tracer.start_as_current_span("split_into_sentences") as span:
            span.set_attribute(SpanAttributes.FI_SPAN_KIND, FiSpanKindValues.TOOL.value)
            span.set_attribute("input.value", json.dumps({"text": text}))
            ...
            span.set_attribute("output.value", json.dumps(sentences))
            return sentences
    ...
    def _get_overlap_sentences(sentences: List[str], overlap_words: int) -> List[str]:
        with tracer.start_as_current_span("get_overlap_sentences") as span:
            span.set_attribute(SpanAttributes.FI_SPAN_KIND, FiSpanKindValues.TOOL.value)
            span.set_attribute("input.value", json.dumps({"sentences": sentences, "overlap_words": overlap_words}))
            ...
            for sentence in reversed(sentences):
                ...
                if word_count + sentence_words <= overlap_words:
                    ...
                    break
            span.set_attribute("output.value", json.dumps(overlap_sentences))
